...
!======================================================================================
   subroutine Detailed_ECalc_Inter(E_T, PairList)
      ...
      real(dp) :: r_ij, r_ik, costheta_ijk
...
!======================================================================================
   subroutine Shift_ECalc_Inter(E_Trial, disp, PairList, dETable, rejMove, update)
      ...
      real(dp) :: rx, ry, rz, r, rx1, ry1, rz1, rx2, ry2, rz2
      real(dp) :: rx1_old, ry1_old, rz1_old, rx2_old, ry2_old, rz2_old
      real(dp) :: rx1_new, ry1_new, rz1_new, rx2_new, ry2_new, rz2_new
      real(dp) :: r_ij, r_ik, r_new, r_old, rmin_ij, r_min1_sq
      real(dp) :: r_ij_old, r_ik_old, r_ij_new, r_ik_new
      ...
      real(dp) :: steric, coulomb, chargeDipole, vanderWaals, twobody, threebody,  threebody_old,  threebody_new
      real(dp) :: rcheta, rcqr1s, rcDr4s, rcW, tmp, r1_dot_r2
      logical :: new1, new2, new3
      integer :: ctr=0, thread_id
      ...
      do iDisp = 1, sizeDisp-1
         ...
         do jType = 1, nMolTypes
            do jDisp = iDisp+1, sizeDisp
               ...
               ! If r_new is greater than r_max reject the move. (avoid dissociation)
               if ((atmType1 .ne.  atmType2) .and. (r_new .gt. 4)) then 
                  rejMove = .true. 
                  return 
               end if 
      ...
      do iType = 1, nMolTypes
         do jType = iType, nMolTypes
            do iiMol = 1, NPART(iType)
               ...
               O_x1_old = MolArray(iType)%mol(iiMol)%x(1)
               O_y1_old = MolArray(iType)%mol(iiMol)%y(1)
               O_z1_old = MolArray(iType)%mol(iiMol)%z(1)

               if (iiMol == dispMol) then
                  new1 = .true.
                  ! NEW OXYGEN COORDINATE
                  O_x1_new = disp(1)%x_new
                  O_y1_new = disp(1)%y_new
                  O_z1_new = disp(1)%z_new
               else
                  new1 = .false.
                  ! NEW OXYGEN COORDINATE
                  O_x1_new = MolArray(iType)%mol(iiMol)%x(1)
                  O_y1_new = MolArray(iType)%mol(iiMol)%y(1)
                  O_z1_new = MolArray(iType)%mol(iiMol)%z(1)
               end if 
               ...
               do jMol = 1, NPART(jType)
                  ...
                  do jAtom = 2, nAtoms(jType)  
                     ...
                     if (jMol == dispMol) then 
                        new2 = .true.
                        rx1_new = O_x1_new - disp(jAtom)%x_new 
                        ry1_new = O_y1_new - disp(jAtom)%y_new
                        rz1_new = O_z1_new - disp(jAtom)%z_new 
                     else
                        new2 = .false.
                        rx1_new = O_x1_new - MolArray(jType)%mol(jMol)%x(jAtom) 
                        ry1_new = O_y1_new - MolArray(jType)%mol(jMol)%y(jAtom) 
                        rz1_new = O_z1_new - MolArray(jType)%mol(jMol)%z(jAtom)
                     end if 
                     r_ij_new = sqrt(rx1_new**2 + ry1_new**2 + rz1_new**2)

                     if (r_ij_new < r0) then
                        do kMol = 1, NPART(jType) ! START OF MOLECULE K
                           if (kMol == dispMol) then
                              new3 = .true. 
                           else 
                              new3 = .false.
                           end if 
                           if (new1 .or. new2 .or. new3) then
                              kIndx = MolArray(jType)%mol(kMol)%indx !?? 
                              ! find another O-H that is within r0
                              do kAtom = 2, nAtoms(jType)
                                 ! NEW 2ND HYDROGEN POSITION
                                 if (((jMol == kMol) .and. (jAtom < kAtom)) .or. (jMol < kMol)) then
                                    if (kMol == dispMol) then 
                                       rx2_new = O_x1_new - disp(kAtom)%x_new
                                       ry2_new = O_y1_new - disp(kAtom)%y_new
                                       rz2_new = O_z1_new - disp(kAtom)%z_new
                                    else 
                                       rx2_new = O_x1_new - MolArray(jType)%mol(kMol)%x(kAtom)
                                       ry2_new = O_y1_new - MolArray(jType)%mol(kMol)%y(kAtom)
                                       rz2_new = O_z1_new - MolArray(jType)%mol(kMol)%z(kAtom)
                                    end if 
                                    r_ik_new = sqrt(rx2_new**2 + ry2_new**2 + rz2_new**2)

                                    if (r_ik_new < r0) then
                                       call threebody_interaction(rx1_new, rx2_new, ry1_new, ry2_new, rz1_new, &
                                                                  rz2_new, r_ij_new, r_ik_new, B, xi, r0, &
                                                                  costheta, threebody_new)
                                       E_threebody_new = E_threebody_new + threebody_new 
                                       dETable(iIndx) = dETable(iIndx) + threebody_new 
                                       dETable(jIndx) = dETable(jIndx) + threebody_new 
                                    end if 
                                 end if 
                              end do 
                           end if 
                        end do ! END OF MOLECULE K
                     end if 

                     ! OLD HYDROGEN POSITION
                     rx1_old = O_x1_old - MolArray(jType)%mol(jMol)%x(jAtom) 
                     ry1_old = O_y1_old - MolArray(jType)%mol(jMol)%y(jAtom) 
                     rz1_old = O_z1_old - MolArray(jType)%mol(jMol)%z(jAtom) 
                     r_ij_old = sqrt(rx1_old**2 + ry1_old**2 + rz1_old**2)
                     ...
                     if (r_ij_old < r0) then
                        do kMol = 1, NPART(jType)
                           ...
                           if (new1 .or. new2 .or. new3) then
                              ...
                              do kAtom = 2, nAtoms(jType)
                                 if (((jMol == kMol) .and. (jAtom < kAtom)) .or. (jMol < kMol)) then
                                    ! OLD 2ND HYDROGEN POSITION
                                    rx2_old = O_x1_old - MolArray(jType)%mol(kMol)%x(kAtom)
                                    ry2_old = O_y1_old - MolArray(jType)%mol(kMol)%y(kAtom)
                                    rz2_old = O_z1_old - MolArray(jType)%mol(kMol)%z(kAtom)
                                    r_ik_old = sqrt(rx2_old**2 + ry2_old**2 + rz2_old**2)
                                    
                                    if (r_ik_old < r0) then
                                       call threebody_interaction(rx1_old, rx2_old, ry1_old, ry2_old, rz1_old, &
                                                                  rz2_old, r_ij_old, r_ik_old, B, xi, r0, &
                                                                  costheta, threebody_old)
                                       E_threebody_old = E_threebody_old  + threebody_old
                                       dETable(iIndx) = dETable(iIndx) - threebody_old
                                       dETable(jIndx) = dETable(jIndx) - threebody_old
                                    end if 
                                 end if 
                              end do
                           end if  
